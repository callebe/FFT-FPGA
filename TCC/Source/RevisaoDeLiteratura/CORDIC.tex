...
\begin{equation}
	K(n)_c~=~\frac{1}{\sqrt{1+ [\mu_{0}2^{-s_0 (n)} + \mu_{1}2^{-s_1 (n)}]^2}}
	\label{eq:EEASRKcn}
\end{equation}
...
\begin{equation}
	\theta_n~=~tan^{-1} \left(\frac{\sum_{i=1}^{I} \mu_{i}2^{-s_i (n)}}{\sum_{j=1}^{J} \mu_{j}2^{-s_j (n)}}\right)
	\label{eq:MSRtheta}
\end{equation}

\begin{equation}
	K(n)~=~\frac{1}{\sqrt{ \left( \sum_{i=1}^{I} 2^{-s_i (n)}\right)^2 + \left(\sum_{j=1}^{J} 2^{-s_j (n)} \right)^2 } }
	\label{eq:MSRKnc}
\end{equation}
...
\begin{eqnarray}
	S_3 = \{ tan^{-1} ( \mu_{0} 2^{-s_0} + \mu_{1} 2^{-s_1}) \} \\ 
	:~\mu_{0},\mu_{1} \in \{-1,0,1\},~s_0,s_1 \in \{0,1, \dots, S\} 
	\label{eq:S3}
\end{eqnarray}
...
Como base na Equação (\ref{eq:variancia}) \citeonline{Chih} analisa o efeito a operação de rotação tem sobre a variância de $e_n$, e mais especificamento o efeito que o ganho $K_c$ tem sobre o SQNR. Quando o ganho $K_c$ é $> ~1$ ou $<~1$ SQNR é reduzido para $1/(K(n))$ a cada interação Cordic.

Assim o ganho $K_C$ é fator importante no projeto dos parâmetros Cordic para o MSR, afetando tanto o erro $e_n$ quanto $\upepsilon$. Assim para refinar a função de minimização apresentada em (\cite{eq:OtimizacaoMSR}), \citeonline{Chih} adiciona a seguinte restrição em relação ao valor do ganho $K_c$:

\begin{equation}
	P_{lower} ~=~1/P_{upper}
\end{equation}

Onde $P_{upper}$ e $P_{lower}$ são respectivamente o limite superior e o limite inferior de $K(n)$ a cada interação.
...
\begin{figure}[H]
	\centering
	\captionsetup{width=0.45\textwidth, font=footnotesize, textfont=bf}	
	\includegraphics[width=0.45\linewidth]{Images/RevisaoDeLiteratura/RelacaoSQNReKc.pdf}
...
O ultimo aspecto da melhoria do erro $e_n$ é a escola dos parâmetros $I$ e $J$. Estes parâmetros estão diretamente relacionados ao número de termos SPT $N$ escolhidos para realizar a operação Cordic, através da Equação . 
