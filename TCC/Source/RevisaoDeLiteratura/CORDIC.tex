...
Um importante fator para a determinação do melhor conjunto de parâmetros Cordic para o MSR, além da minimização do erro  $\upepsilon$, é o efeito do ruído de arredondamento (\textit{Roundoff Noise Analysis} - RNA), e o erro de \textit{overflow}, ou estouro da palavra binária. Ambos os efeitos ocorrem ao longo das operações lógicas promovidas pela algoritmo Cordic, e que também podem ocorrer fora do bloco lógico Cordic e que acabam sendo amplificados por ele.  
...
\begin{equation}
	\left{-2^{W-i-1},...,-2^{-i+1}, -2^{-i}, -2^{-i+1}, ..., 2^{W-i-1}}  
\end{equation}

Onde $W$ representa o numero de \textit{bits} da palavra binária, e $i$ representa o número de dígitos fracionários.

Uma solução simples para reduzir o efeito do RNA, é aumentar o número de \textit{bits} $W$ e $i$, melhorando a resolução da representação. Porém esta solução, como afirma \cite{Chih}, provocaria uma redução na velocidade computacional do sistema, e ainda consumiria mais recursos de \textit{hardware}.
